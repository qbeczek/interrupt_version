...
// Deskryptory dla endpointów interrupt
#define INTERRUPT_OUT_EP_ADDR 0x01
#define INTERRUPT_IN_EP_ADDR 0x81
...
void i2c_work_handler(struct k_work *work) {
    ...
    if (i2c_cmd.operation == 0) {  // Zapis
        ...
        int result = 0;
        uint8_t ep = 0x81;
        result = usb_write(ep, i2c_cmd.data, 1, NULL);
        if (result == 0) {
...
static void interrupt_ep_out_cb(uint8_t ep,
                                enum usb_dc_ep_cb_status_code ep_status) {
    uint32_t bytes_to_read;
    printk("interrupt_ep_out_cb_________________\n");

    // Odczytaj ilość bajtów do odczytu
    int ret = usb_read(ep, NULL, 0, &bytes_to_read);
    if (ret < 0) {
        LOG_ERR("Failed to get bytes to read: %d", ret);
        return;
    }
    ...
    // Odczytaj dane z bufora USB do struktury
    ret = usb_read(ep, (uint8_t *)&i2c_cmd, bytes_to_read, NULL);
    if (ret < 0) {
        LOG_ERR("Failed to read USB data: %d", ret);
        return;
    }
...
static void interrupt_ep_in_cb(uint8_t ep,
                               enum usb_dc_ep_cb_status_code ep_status) {
    uint8_t stalled;
    printk("interrupt_ep_in_cb_________________\n");
    int result = usb_dc_ep_is_stalled(ep, &stalled);
    LOG_WRN("usb_dc_ep_is_stalled - return: %d, stalled: %d", result, stalled);

    result = usb_write(ep, i2c_cmd.data, i2c_cmd.data_length, NULL);

    if (result == 0) {
        LOG_INF("ep 0x%x", ep);
        LOG_INF("Sending value: %x", i2c_cmd.data[0]);
    } else {
        LOG_ERR("USB write failed with error code %d", result);
    }
}

static struct usb_ep_cfg_data ep_cfg[] = {
    {
        .ep_cb = interrupt_ep_out_cb,
        .ep_addr = INTERRUPT_OUT_EP_ADDR,
    },
    {
        .ep_cb = interrupt_ep_in_cb,
        .ep_addr = INTERRUPT_IN_EP_ADDR,
    },
};
...
static void usb_status_cb(struct usb_cfg_data *cfg,
    ...
    switch (status) {
        case USB_DC_INTERFACE:
            interrupt_ep_in_cb(ep_cfg[BULK_IN_EP_IDX].ep_addr, 0);
